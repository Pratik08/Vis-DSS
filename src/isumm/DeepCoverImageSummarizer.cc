...
DeepCoverImageSummarizer::DeepCoverImageSummarizer(std::vector<cv::Mat>& ImageCollection, CaffeClassifier& cc, std::string featureLayer,
  int summaryFunction, int FeatureBasedFnType, bool debugMode) : ImageCollection(ImageCollection), cc(cc), featureLayer(featureLayer), summaryFunction(summaryFunction),
  FeatureBasedFnType(FeatureBasedFnType), debugMode(debugMode)
{
