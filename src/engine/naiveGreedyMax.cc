...
bool isEquality, Set groundSet, bool grSetn)
{
    ...
    if(verbosity > 0) std::cout<<"Starting the naive greedy algorithm\n"<<std::flush;
    double currvalbest;
    int curridbest;
    ...
    while (1)
    {
        ...
        if (verbosity > 0) std::cout<<"Current best value is "<<currvalbest<<"\n"<<std::flush;
        if(currentCost <= budget)
        {
            ...
            if(verbosity > 0) std::cout<<"Adding element "<< curridbest<<" in iteration "<<iter<<" and the objective value is "<<f.eval(greedySet)<<"\n"<<std::flush;
        }
        else{
            break;
        }
...
bool isEquality, Set groundSet, bool grSetn)
{
    ...
    while (1)
    {
        ...
        if (verbosity > 0) std::cout<<"Current best value is "<<currvalbest<< " and id is " << curridbest << "\n"<<std::flush;
