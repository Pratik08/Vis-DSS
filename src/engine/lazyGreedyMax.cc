...
void lazyGreedyMax(const SetFunctions& f, double budget, Set& greedySet, int verbosity, bool precomputeSet, bool equalityConstraint, Set groundSet, bool grSetn) {
    ...
    std::priority_queue <Increment> rho;
    // initilize the priority queue
    if (verbosity > 0) {
        std::cout << "Start Initializing the priority queue" << std::endl;
    }
    if (grSetn) {
...
void lazyGreedyMaxSC(SetFunctions& f, std::vector<double>& costList, double cover, Set& greedySet, int verbosity,
    ...
    while (!rho.empty()) {
        ...
        if (newV < rho.top().get_value()) {
            ...
            if (objV < cover * fV) {
                ...
                currentCost += costList[topid];
                if (verbosity >= 0) {
                    printf("Selecting %dth sample, curCost/cover = %.6e/%.6e, preV = %.10e, curV = %.10e, num_resort=%d\n", topid, currentCost, cover * fV, preV, maxV, sort_cnt);
                }
                preV = maxV;
                f.updateStatisticsAdd(greedySet, topid);
                sort_cnt = 0;
