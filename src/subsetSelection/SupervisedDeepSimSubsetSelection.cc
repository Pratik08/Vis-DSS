...
SupervisedDeepSimSubsetSelection::SupervisedDeepSimSubsetSelection(std::vector<cv::Mat>& ImageCollection, std::vector<std::string>& ImageCollectionName, std::vector<int>& ImageCollectionLabels, CaffeClassifier& cc, std::string featureLayer, int summaryFunction, bool debugMode) : ImageCollection(ImageCollection), ImageCollectionName(ImageCollectionName), ImageCollectionLabels(ImageCollectionLabels), cc(cc), featureLayer(featureLayer), summaryFunction(summaryFunction), debugMode(debugMode) {
...
void SupervisedDeepSimSubsetSelection::extractFeatures() {
...
void SupervisedDeepSimSubsetSelection::computeKernel(int compare_method) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < n; j++) {
            float val;
            if (ImageCollectionLabels[i] == ImageCollectionLabels[j]) {
                if (compare_method == 0) {
...
void SupervisedDeepSimSubsetSelection::summarizeBudget(int budget) {
    ...
    if (summaryFunction == 0) {
        ...
        std::cout << "Supervised DSS is not supported for MMR Function\n";
        ...
        std::cout << "Supervised DSS is not supported for Graph Cut Function\n";
    } else if (summaryFunction == 4) {
        std::cout << "Supervised DSS is not supported for Saturated Coverage Function\n";
    }
    std::cout << "Summarization is done with n = " << n << " and budget = " << budget << "\n" << std::flush;
}

void SupervisedDeepSimSubsetSelection::summarizeStream(double epsilon) {
    Set optSet;
    if (summaryFunction == 0) {
        DisparityMin dM(n, kernel);
        optSet.insert(0);
...
void SupervisedDeepSimSubsetSelection::summarizeCover(double coverage) {
    Set optSet;
    if (summaryFunction == 0) {
...
void SupervisedDeepSimSubsetSelection::writeSummarizedImagesInFile(char* subsetFilePath, int budget) {
    ...
    for (std::set<int>::iterator it = summarySet.begin(); it != summarySet.end(); it++) {
      ...
      outfile << ImageCollectionName[*it] << " " << ImageCollectionLabels[*it] << "\n";
    }
}
