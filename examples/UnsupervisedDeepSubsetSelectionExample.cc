...
Arg Arg::Args[] = {
    Arg("supersetFilePath", Arg::Req, supersetFilePath, "superSetTrainTxt", Arg::SINGLE),
...
int main(int argc, char** argv) {
    ...
    if (simcover == 0) {
        UnsupervisedDeepSubsetSelection DSS(ImageCollection, ImageCollectionName, cc, featureLayer, summaryFunctionSim, false);
        DSS.extractFeatures();
        DSS.computeKernel();
        if (summaryAlgo == 0) {
